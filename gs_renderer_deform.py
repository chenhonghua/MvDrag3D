...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        # deform related
        self._deformation_pos = deform_network() 
        self._deformation_shs = deform_network() 
        # record the id for each view
    ...
    def capture(self):
        return (
            ...
            self._xyz,
            self._deformation_pos.state_dict(),
            self._deformation_shs.state_dict(),
    ...
    def restore(self, model_args, training_args):
        ...
        self._xyz, 
        self._deformation_table,
        self._deformation_pos, 
        self._deformation_shs,
        self._features_dc, 
        self._features_rest,
        self._scaling, 
        self._rotation, 
    ...
    def pos_deform(self):
        ...
        mask0 = self._deformation_table == 0
        mask1 = self._deformation_table == 1
        mask2 = self._deformation_table == 2
        mask3 = self._deformation_table == 3
        ...
        gs0_deform, gs1_deform, gs2_deform, gs3_deform = self._deformation_pos(gs0, gs1, gs2, gs3)
        gs_deform = torch.cat((gs0_deform, gs1_deform, gs2_deform, gs3_deform), dim=0)

        return gs_deform
    
    def shs_deform(self):
        # deform shs
        features = self.get_features.detach()
        mask0 = self._deformation_table == 0
        mask1 = self._deformation_table == 1
        mask2 = self._deformation_table == 2
        mask3 = self._deformation_table == 3

        sh0 = features[mask0[:,0],:,:]
        sh1 = features[mask1[:,0],:,:]
        sh2 = features[mask2[:,0],:,:]
        sh3 = features[mask3[:,0],:,:]

        sh0_deform, sh1_deform, sh2_deform, sh3_deform = self._deformation_shs(sh0, sh1, sh2, sh3)
        sh_deform = torch.cat((sh0_deform, sh1_deform, sh2_deform, sh3_deform), dim=0)

        return sh_deform # sh_deform
    ...
    def update_optimizer(self, training_args):
        l = [
    ...
    def training_setup(self, training_args):
        self._deformation_pos = self._deformation_pos.to("cuda")  
        self._deformation_shs = self._deformation_shs.to("cuda")  
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")

        if training_args.optimize_appearance:
            ...
            self._opacity.requires_grad = True
            l = [
                {'params': list(self._deformation_pos.get_mlp_parameters()), 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "pos_deformation"},
                {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
                {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
                {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
                {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
            ...
            l = [
                {'params': list(self._deformation_pos.get_mlp_parameters()), 'lr': training_args.deformation_lr_init * self.spatial_lr_scale, "name": "pos_deformation"}
                # {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
                # {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            ]
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
                return lr
            if param_group["name"] == "pos_deformation":
    ...
    def save_deformation(self, path, name):
        torch.save(self._deformation_pos.state_dict(),os.path.join(path, name+"_deformation_pos.pth"))
        torch.save(self._deformation_shs.state_dict(),os.path.join(path, name+"_deformation_shs.pth"))
        torch.save(self._deformation_table,os.path.join(path, name+"_deformation_table.pth"))

    def save_ply(self, path, gs_deform=None):
        os.makedirs(os.path.dirname(path), exist_ok=True)

        xyz = self._xyz.detach().cpu().numpy()
        if gs_deform is not None:
            gs_deform = gs_deform.detach().cpu().numpy()
            xyz += gs_deform
        ...
        view_id = self._deformation_table.detach().cpu().numpy() 

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        ### we do not renew the view_id when doing clone and split
        if view_id.shape[0] != xyz.shape[0]:
            view_id = np.zeros((xyz.shape[0], 1))
        
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, view_id), axis=1)
    ...
    def apply_grad_mask(self, mask):
        ...
        fields = ["_xyz", "_features_dc", "_features_rest", "_opacity", "_rotation", "_scaling"]
    ...
    def load_ply(self, path, is_optimize=True, is_xyz = True):
        ...
        # load view_id
        if plydata.elements[0]['view_id'] is not None:
            viewid = np.asarray(plydata.elements[0]["view_id"])[..., np.newaxis]
            self._deformation_table = torch.tensor(viewid, dtype=torch.float, device="cuda") 
    ...
    def update_editing_mask(self):
        xyz_np = self._xyz.detach().cpu().numpy()
        kdtree = cKDTree(xyz_np)
        editing_mask = torch.zeros((self.get_xyz.shape[0]), dtype=bool, device="cuda")
        for i in range( self.center_points.shape[0]):
            center_point =  self.center_points[i]
            indices = kdtree.query_ball_point(center_point, self.mask_radius)
            indices = np.array(indices, dtype=int)
            editing_mask[indices] = True
        self.set_mask(editing_mask)
...
class Renderer:
    def __init__(self, sh_degree=3, white_background=True, radius=1):
        ...
        self.gaussians_edited = GaussianModel(sh_degree)
        # self.gaussians_fix = GaussianModel(sh_degree)
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        ...
        if input is None:
            ...
            # load from saved ply
            self.gaussians.load_ply(input, is_optimize=False, is_xyz=False)
    ...
    def render(
        ...
        if gs_deform is not None:
            means3D = self.gaussians.get_xyz + gs_deform
        else:
            means3D = self.gaussians.get_xyz
        means2D = screenspace_points
        if op_deform is not None:
            opacity = self.gaussians.get_opacity + op_deform
        else:
        ...
        if colors_precomp is None:
            if convert_SHs_python:
                ...
                if sh_deform is not None:
                    shs = self.gaussians.get_features + sh_deform
                else:
