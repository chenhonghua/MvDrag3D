...
class FixedDataset(Dataset):
    ...
    def __init__(self, opt: Options, training=True):
        ...
        # load image
        # image_path = "/mnt/slurm_home/hhchen/LGM/crocodile_noscale/test_imagedream_drag.png"
        # image_path = "/mnt/slurm_home/hhchen/LGM/lionhead_noscale/test_imagedream_drag2_copy.png"
        # image_path = "/mnt/slurm_home/hhchen/LGM/martin_boot_noscale/test_imagedream_drag2.png"
        image_path = self.opt.mv_image
        input_image = kiui.read_image(image_path, mode='uint8')
        ...
        self.mv_image = image.reshape(2, 256, 2, 256, 3).transpose(0, 2, 1, 3, 4).reshape(4, 256, 256, 3) #(4, 256, 256, 3)
        self.mask = mask.reshape(2, 256, 2, 256, 1).transpose(0, 2, 1, 3, 4).reshape(4, 256, 256, 1) #(4, 256, 256, 1)
        print('---------self.mv_image.shape: ', self.mv_image.shape)
    ...
    def __getitem__(self, idx):
        ...
        # load num_views images
        images = torch.from_numpy(self.mv_image).permute(0, 3, 1, 2) # [4, 3, 256, 256] (0~1)
        masks = torch.from_numpy(self.mask).permute(0, 3, 1, 2) #torch.Size([4, 1, 256, 256])
        masks = masks.squeeze(1) #torch.Size([4, 256, 256])
            
        # load camera pose
        cam_poses = []
        azimuth = np.arange(0, 360, 90, dtype=np.int32)
        for azi in tqdm.tqdm(azimuth):
            c2w = torch.from_numpy(orbit_camera(0, azi, radius=self.opt.cam_radius, opengl=True)).unsqueeze(0) #torch.Size([1, 4, 4])
            # c2w[:, :3, 1:3] *= -1 # invert up & forward direction
            cam_poses.append(c2w)
          
        cam_poses = torch.stack(cam_poses, dim=0) # [V, 1, 4, 4]
        cam_poses = cam_poses.squeeze(1) # [V, 4, 4]
